...
defaultConfig {
    applicationId = project.findProperty("nativephp.app_id")?.toString()
        ?: "com.mohamedmohamed.swiftgoldenlunar"
    minSdk = 33
    targetSdk = 36
    versionCode = (project.findProperty("nativephp.version_code")?.toString()?.toIntOrNull() ?: 5)
    versionName = project.findProperty("nativephp.version_name")?.toString() ?: "1.0.5"
...
buildTypes {
    release {
        isMinifyEnabled = (project.findProperty("nativephp.minify_enabled")?.toString()?.toBooleanStrictOrNull() ?: false)
        isShrinkResources = (project.findProperty("nativephp.shrink_resources")?.toString()?.toBooleanStrictOrNull() ?: false)
        ...
        ndk {
            debugSymbolLevel = project.findProperty("nativephp.debug_symbols")?.toString() ?: "FULL"
        }
    }
    debug {
